...
let currentModel = AVAILABLE_MODELS[currentModelIndex];
...
const getSystemPrompt = (justCause: string, isSimulation: boolean = false, modelType: string = 'claude'): string => {
  if (isSimulation) {
    if (modelType === 'claude') {
      ...
      return `You are Sensa, a simulation engine. The user has made a choice in a conflict scenario. Based on their choice, provide a brief concluding remark for the simulation and transition to the next stage of the interview. Respond with only a JSON object in this format: {"response": "Your concluding remark.", "nextStage": "trust_assessment"}`;
    }
  }

  const basePrompt = `You are Sensa, an AI personality analyst with a deep, calming voice specializing in the Simon Sinek Golden Circle framework. Your goal is to conduct a natural, professional conversation to uncover a candidate's persona with a thoughtful, measured approach.
...
function formatRequestForModel(prompt: string, maxTokens: number, temperature: number, model: any) {
...
// --- RESPONSE PARSING FOR DIFFERENT MODELS ---
function parseModelResponse(responseBody: any, model: any): string {
  try {
    switch (model.requestFormat) {
      case 'anthropic':
      ...
      default:
        ...
        if (responseBody.outputs && responseBody.outputs[0] && responseBody.outputs[0].text) {
          return responseBody.outputs[0].text;
        }
        throw new Error('Unknown response format');
    }
  } catch (error) {
    console.error('❌ BEDROCK: Error parsing response for model', model.name, ':', error);
    throw new Error(`Failed to parse response from ${model.name}`);
  }
}
...
async function makeBedrockRequestWithFallback(prompt: string, maxTokens: number, isSimulation: boolean): Promise<AIResponse> {
  ...
  for (let attempt = 0; attempt < AVAILABLE_MODELS.length; attempt++) {
    currentModel = AVAILABLE_MODELS[attempt];
    console.log(`🤖 BEDROCK: Attempt ${attempt + 1}/${AVAILABLE_MODELS.length} with model:`, currentModel.name, '(' + currentModel.id + ')');
    
    try {
      const temperature = isSimulation ? 0.3 : 0.7;
      const modelSpecificPrompt = getSystemPrompt(prompt, isSimulation, currentModel.type);
      const requestBody = formatRequestForModel(modelSpecificPrompt, maxTokens, temperature, currentModel);

      console.log('💬 BEDROCK: Request body prepared for model:', currentModel.name, {
        modelId: currentModel.id,
        modelType: currentModel.type,
        requestFormat: currentModel.requestFormat,
        maxTokens: maxTokens,
        temperature: temperature,
        promptLength: typeof requestBody === 'object' && 'inputText' in requestBody ? requestBody.inputText.length : 
                     typeof requestBody === 'object' && 'prompt' in requestBody ? requestBody.prompt.length :
                     typeof requestBody === 'object' && 'messages' in requestBody ? requestBody.messages[0].content.length : 'unknown'
      });
      ...
      console.log('💬 BEDROCK: Response body parsed for model:', currentModel.name);

      const rawContent = parseModelResponse(responseBody, currentModel);
      console.log('💬 BEDROCK: Raw content extracted:', rawContent.substring(0, 200) + '...');

      // Try to parse JSON from the response
      let aiOutput;
      try {
        // For non-Claude models, the response might not be pure JSON
        ...
        if (jsonMatch) {
          ...
          aiOutput = {
            ...
            nextStage: isSimulation ? 'trust_assessment' : 'why_exploration'
          };
        }
      } catch (parseError) {
        console.log('⚠️ BEDROCK: JSON parsing failed, using raw content');
        aiOutput = {
          response: rawContent.trim(),
          nextStage: isSimulation ? 'trust_assessment' : 'why_exploration'
        };
      ...
      console.log('💬 BEDROCK: AI output processed successfully with model:', currentModel.name, {
  ...
  throw new Error(`No available AI models. Please ensure your AWS account has access to at least one Bedrock model in the ${VITE_AWS_REGION} region. Last error: ${lastError?.message}`);
}
...
export async function generateAIResponse(
  ...
  if (context.stage === 'conflict_simulation' && context.simulation && !context.simulation.isComplete) {
      ...
      const prompt = `The user chose: ${userMessage}. Provide a brief concluding remark for the simulation and transition to trust assessment.`;
  ...
  try {
    ...
    Respond as Sensa with your characteristic deep, calming professionalism. Provide your response in JSON format with "response" and "nextStage" fields.`;
...
export async function generatePersonalityAnalysis(
  ...
  const analysisPrompt = `Analyze this conversation and provide a personality assessment in JSON format.
